package SuiteRunners::PerformanceEditmode;
...
my $suiteOptions = [
    ...
    [
        'projectlist:s',
        'File contains list of projects with editmode tests',
        {
            defaultValue => $projectListFileName
        }
    ],
    ['categories:s@', "Filter tests by categories e.g. 'QV'"],
    [
        'platform:s',
...
sub beforeRun
{
    ...
    #editmode tests are not doing any prepare
...
sub usage
{
    ...
    return (
        name => $this->getName(),
        description => 'Performance editmode tests.',
        url => 'http://internaldocs.hq.unity3d.com/automation/TestFrameworks/PerformanceTests/PlaymodeAndEditmode/index.html',
        examples => [{ description => 'Run ABV editmode tests', command => '--categories=ABV' }]
    );
}

sub getName
{
    return 'performance-editmode';
}
