...
namespace UnderlyingModel.Tests
{
    ...
    public partial class NewModelTests
    {
        ...
        [TestCase("Array._length", "Array-length")] // DocOnly property
        [TestCase("Array", "Array")] //DocOnly class
        ...
        {
            MemberItem member = m_NewDataItemProject.GetMember("UnityEngine");
            Assert.IsNotNull(member);
            Assert.AreEqual("UnityEngine", member.ItemName);
            Assert.AreEqual("UnityEngine", member.HtmlName);
            Assert.AreEqual("UnityEngine", member.DisplayName);
            Assert.AreEqual(AssemblyType.Assembly, member.ItemType);
            Assert.Contains("Vector3", member.ChildMembers.Select(m => m.ItemName).ToArray());
        }
