...
namespace Unity.ReferenceRewriter
{
    class RemoveStrongNamesFromAssemblyReferences : RewriteStep
    {
        ...
        private bool ShouldRemoveStrongName(AssemblyNameReference reference)
        {
            ...
            var assemblyDir = NormalizePath(Path.GetDirectoryName(assembly.MainModule.FileName));
            if (Context.FrameworkPaths.Any(path => NormalizePath(path) == assemblyDir))
            {
                return false;
            }
            var nuGetResolver = Context.AssemblyResolver as NuGetAssemblyResolver;
            if (nuGetResolver != null)
            {
                if (nuGetResolver.IsFrameworkAssembly(reference))
                    return false;
            }
